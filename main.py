...
def main():
    ...
    stereoMatcher = SSM.StereoMatcher(SSM.STEREO_TYPE_BM,96,15)
    ...
    R1, R2, P1, P2, Q, roi_left, roi_right = cv.stereoRectify(K1, D1, K2, D2, IMGSIZE, R, T ,alpha=-1)
    ...
    while True:
        ...
        cv.imshow('disparity',disparity/ np.max(disparity))
        cv.imshow('filteredDisparity',filteredDisp/np.max(filteredDisp))
        cv.imshow('depth',image3D[:,:,2]/np.max(image3D[:,:,2]))
